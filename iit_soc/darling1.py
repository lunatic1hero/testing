...
#we are opening the log file here

f = open('http_log.csv',"w")
c = csv.writer(f)
...
for items in result:
   ...
   rr = base64.b64decode(items).decode('utf-8')  # here the raw http req is decode with help of decoder and stored in rr
   method,body,path,headers =  rhrp(rr)
   data.append(method)
   data.append(body)
   data.append(path)
   data.append(headers)
   f = open('http_log.csv',"ab")
   c = csv.writer(f)
   c.writerow(data)
   f.close()
